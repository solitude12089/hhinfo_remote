...
class RemoteController extends Controller {
		...
		try{
			...
			if($spcard!=null){
				if(in_array($device->family,$spcard->family)&&$device->group_id ==$spcard->group_id){
					...
					//return $this->opendoor($device,$senser1);
					return $this->opendoor($device,$senser1,$customer->id,$e_swipe_event->id,'全區卡');
				}
			}
	...
	public function opendoor($device,$senser,$customer_id,$e_swipe_event_id,$mode){
